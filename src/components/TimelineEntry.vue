...
<template>
  ...
  >
    ...
    <div class="flex items-start justify-between gap-2 mb-2">
      <h3 style="font-family: var(--font-sans); font-size: var(--text-base); font-weight: 600; color: var(--text-primary); line-height: 1.3; margin-bottom: 4px; white-space: normal; overflow: visible; display: block; width: 100%; word-break: break-word;">
        {{ t(`timeline.entries.${entry.id}.title`) }}
      </h3>
      <span
        class="shrink-0"
        style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); color: var(--text-secondary); font-family: var(--font-sans); font-size: var(--text-xs); text-transform: uppercase; letter-spacing: 0.08em; border-radius: 4px; padding: 2px 8px;"
      >
    ...
    <!-- Org + Location + Date -->
    <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mb-2" style="font-size: var(--text-xs); color: var(--text-secondary);">
      <span>{{ entry.org }}</span>
    ...
    <div v-if="entry.stack?.length" class="flex flex-wrap gap-1 mb-2">
      <span
        ...
        style="background: rgba(240,240,240,0.06); border: 1px solid rgba(240,240,240,0.15); color: var(--text-primary); font-family: var(--font-mono); font-size: var(--text-xs); padding: 2px 8px; border-radius: 4px;"
    ...
    <a
      ...
      style="color: var(--accent); font-size: var(--text-xs); text-decoration: none;"
